...
namespace EntityFrameworkCore.Repositories
{
    ...
    public abstract class RepositoryBase<TEntity, TPrimaryKey> : IRepository<TEntity, TPrimaryKey> where TEntity : Entity<TPrimaryKey>
    {
        ...
        /// <returns></returns>
        public TEntity FirstOrDefalut(Expression<Func<TEntity, bool>> predicate)
        {
        ...
        private void EntityToEntity<T>(T pTargetOjbSrc,T pTargetOjbDest)
        {
            foreach (var mItem in typeof(T).GetProperties())
            {
                mItem.SetValue(pTargetOjbDest, mItem.GetValue(pTargetOjbSrc, new object[] { }),null);
            }
        }
        /// <summary>
        /// 根据主键构建判断表达式
        ...
        {
            var lambdaParan = Expression.Parameter(typeof(TEntity));
            var lambdaBody = Expression.Equal(
                Expression.PropertyOrField(lambdaParan,"Id"),
                Expression.Constant(id,typeof(TPrimaryKey))
                );
            return Expression.Lambda<Func<TEntity, bool>>(lambdaBody, lambdaParan);
        }
    }
    ...
    {
        public RepositoryBase(RoleDBContext dbContext) : base(dbContext)
        {
